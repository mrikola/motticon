...
export class FixPickedCardReference1715619505111 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "picked_card" DROP COLUMN "cardlistId"`,
    );
    await queryRunner.query(`ALTER TABLE "listed_card" DROP COLUMN "pickedId"`);
    await queryRunner.query(
      `ALTER TABLE "picked_card" ADD "listedCardId" integer`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "picked_card" DROP COLUMN "listedCardId"`,
    );
    await queryRunner.query(`ALTER TABLE "listed_card" ADD "pickedId" integer`);
    await queryRunner.query(
      `ALTER TABLE "picked_card" ADD "cardlistId" integer`,
    );
    await queryRunner.query(`ALTER TABLE "picked_card" ADD "cardId" integer`);
    await queryRunner.query(
      `ALTER TABLE "listed_card" ADD CONSTRAINT "FK_0d87dbce02131b3876c0a47cc96" FOREIGN KEY ("pickedId") REFERENCES "picked_card"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
