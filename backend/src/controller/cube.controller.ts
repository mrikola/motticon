...
export class CubeController extends Controller {
    ...
    @Security('loggedIn')
    public async getCubesForTournament(@Path() tournamentId: number): Promise<CubeDto[]> {
        return (await this.cubeService.getCubesForTournament(tournamentId)).map(cubeToDto);
    }
    ...
    public async addCube(
        ...
        return cubeToDto(
            await this.cubeService.addCube(title, description, url, owner, imageUrl, cards)
        );
    }
