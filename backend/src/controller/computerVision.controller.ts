...
export class ComputerVisionController extends Controller {
  ...
  public async getListedCardsFromImageUrl(
    @Body() data: { url: string; cubeCards: ListedCard[] },
  ): Promise<ComputerVisionDto> {
    const { url, cubeCards } = data;
    return await this.computerVisionService.getListedCardsFromImageUrl(
      url,
      cubeCards,
    );
  }
  ...
  public async textsToListedCards(
    @Body() data: { rawTexts: ComputerVisionDto; dictionary: ListedCard[] },
  ): Promise<ComputerVisionDto> {
    const { rawTexts, dictionary } = data;
    return await this.computerVisionService.textsToListedCards(
      rawTexts,
      dictionary,
    );
  }
