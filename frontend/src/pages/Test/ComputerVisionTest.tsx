...
const ComputerVisionTest = () => {
  ...
  // function to mass add cards to Deck object
  const deckFromCards = (cards: string[]) => {
    const tempDeck: Deck = new Deck();
    for (const card of cards) {
      tempDeck.addCard(card, false);
    }
    return tempDeck;
  };

  // set this search url through form input, has a default image url stored
  const [searchUrl, setSearchUrl] = useState<string>(
    "https://user-images.githubusercontent.com/49362475/105632710-fa07a180-5e54-11eb-91bb-c4710ef8168f.jpeg"
  );
  // update search url when input changes
  const searchUrlChanged = (url: string) => {
    setSearchUrl(url);
  };

  const textToCardsTest = async () => {
    const rawTexts = [
      "Blakk Lotuss",
      "Moxy Opaly",
      "Sol Bing",
    ...
    try {
      post("/computerVision/textsToCards", { rawTexts, dictionary }).then(
        async (resp) => {
  ...
  const urlToTextsTest = async () => {
    console.log("sent cv request with url: " + searchUrl);
    const url = searchUrl;
    try {
      post("/computerVision/textFromImageUrl", { url }).then(async (resp) => {
        const data = (await resp.json()) as string[];
        console.log(data);
      });
    } catch (error) {
      ...
      <Container>
        ...
        {/* <Row>
          <Button
            ...
            onClick={() => textToCardsTest()}
          >
            Fire text-to-cards test
          </Button>
        </Row>
        <Row>
          <Button
        ...
        <Row>
          <Col xs={12}>
            <FloatingLabel controlId="url" label="URL" className="mb-3">
              <Form.Control
                defaultValue="https://user-images.githubusercontent.com/49362475/105632710-fa07a180-5e54-11eb-91bb-c4710ef8168f.jpeg"
                type="url"
                placeholder="abc123"
                onChange={(e) => searchUrlChanged(e.target.value)}
              />
            </FloatingLabel>
          </Col>
          <Col xs={12} className="d-grid gap-2 mx-auto">
            <Button
              variant="primary"
              className="btn-lg my-3"
              onClick={() => cardsFromImageTest()}
            >
              Cards-from-image test
            </Button>
          </Col>
        </Row>
        <Row>
...
export default ComputerVisionTest;
